...
# $Header: /var/cvsroot/gentoo-x86/net-misc/memcached/memcached-1.4.13.ebuild,v 1.1 2012/02/21 09:02:40 robbat2 Exp $
...
IUSE="test slabs-reassign repcached debug sasl" # hugetlbfs later
...
src_prepare() {
	...
	if use repcached ; then
		epatch "${FILESDIR}/${REP_P}-${PV}.patch"
	fi
...
src_configure() {
	...
	use repcached && myconf="${myconf} --enable-replication"
...
src_install() {
	...
	newconfd "${FILESDIR}"/1.4.13-rep/conf memcached
	newinitd "${FILESDIR}"/1.4.13-rep/init memcached
}
